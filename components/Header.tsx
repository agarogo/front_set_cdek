...
const Header: React.FC<HeaderProps> = ({ currentUser }) => {
  ...
  const isActive = (route: string) =>
    path === route ? "text-[#00B33C]" : "text-black";

  // ---- Навигация ----

  const goDashboard = () => router.push("/news");
  const goChatBot = () => currentUser.role === "user" && router.push("/dock");
  const goDiary = () => currentUser.role === "user" && router.push("/dashboard");
  const goTest = () => currentUser.role === "user" && router.push("/test");
  const goRecs = () =>
    currentUser.role === "user" && router.push("/notifications");
  const goCreateUser = () =>
    currentUser.role === "admin" && router.push("/mail");
  const goCommunity = () => router.push("/chat");
  ...
  return (
    <header className="bg-white shadow-md">
      ...
      <div className="max-w-[90%] mx-auto px-4 py-3 flex items-center justify-between gap-4">
          ...
          <nav className="hidden md:flex items-center gap-4 lg:gap-6 text-sm lg:text-base">
            {currentUser.role === "user" && (
              <>
                <button
                  onClick={goDashboard}
                  className={`hover:text-[#00B33C] ${isActive("/news")}`}
                >
                  Дэшборд
                </button>
                <button
                  onClick={goChatBot}
                  className={`hover:text-[#00B33C] ${isActive("/dock")}`}
                >
                  Чат-бот
                </button>
                <button
                  onClick={goDiary}
                  className={`hover:text-[#00B33C] ${isActive("/dashboard")}`}
                >
                  Дневник
                </button>
                <button
                  onClick={goRecs}
                  className={`hover:text-[#00B33C] ${isActive(
                    "/notifications"
                  )}`}
                >
                  Рекомендации
                ...
                <button
                  onClick={goTest}
                  className={`hover:text-[#00B33C] ${isActive("/test")}`}
                >
                  Опрос
                </button>
              </>
            ...
            {currentUser.role === "admin" && (
              <>
                ...
                <button
                  ...
                  className={`hover:text-[#00B33C] ${isActive("/mail")}`}
                >
            ...
            <button
              onClick={goCommunity}
              className={`hover:text-[#00B33C] ${isActive("/chat")}`}
            >
              Сообщество
            </button>
          </nav>
        ...
        {/* Правый блок: Фамилия И.О., аватар и бургер */}
        ...
        <div className="md:hidden border-t bg-white">
          <nav className="max-w-[90%] mx-auto py-3 flex flex-col gap-2 text-sm">
            ...
            <button
              ...
              className={`text-left py-1 ${isActive("/chat")}`}
            >
              Сообщество
            </button>
          </nav>
      ...
      {/* Модалка профиля — упрощённый формат, но с полями */}
      {isProfileOpen && (
        <div
          ...
          >
            ...
            <div className="grid sm:grid-cols-2 gap-4 text-sm">
              ...
              <div className="space-y-1">
                ...
                <p className="text-gray-500 mt-3">Аккаунт активен</p>
                <p className="font-medium">
                  {currentUser.is_active ? "Да" : "Нет"}
                </p>

                <p className="text-gray-500 mt-3">Попытки входа</p>
                <p className="font-medium">
                  {currentUser.login_attempts ?? "N/A"}
                </p>
              </div>
            </div>
          </div>
